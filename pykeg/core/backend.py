...
class KegbotBackend:
	...
	def GetAuthToken(self, auth_device, token_value):
		...
		try:
			tokenReady = models.AuthenticationToken.objects.get(auth_device=auth_device, token_value=token_value)
			tokenReady.user.total = get_total_for_user(tokenReady.user)
			print tokenReady.payment
			if (tokenReady.payment == "PRE"):
				print tokenReady.user.total
				if (tokenReady.user.total <= 0):
					print tokenReady.user.total
					raise NoTokenError("CreditLimitError")
			return tokenReady
	...
	@transaction.commit_on_success
	def StartKeg(self, tap, beer_type=None, keg_size=None, brewer_name=None, beer_name=None, style_name=None):
