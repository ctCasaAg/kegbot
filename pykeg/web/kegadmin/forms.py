...
class ChangeKegForm(forms.Form):
	...
	description = forms.CharField(required=False, help_text='Public description of this specific keg (optional)')
	...
	def save(self, tap):
		...
		keg = b.StartKeg(tap, beer_name=cd['beer_name'], brewer_name=cd['brewer_name'], style_name=cd['style_name'], keg_size=cd['keg_size'])
...
class BeerTypeForm(forms.ModelForm):
	...
	helper.layout = Layout(
			...
			FormActions(
					Submit('submit_beer_type', 'Save', css_class='btn-primary'),
			)
	)

class DeleteBeerTypeForm(forms.Form):
	helper = FormHelper()
	helper.form_class = 'form-horizontal'
	helper.layout = Layout(
		FormActions(
			Submit('submit_delete_beer_type_form', 'Delete Beer Style', css_class='btn-danger'),
		)
	)
...
class BrewerForm(forms.ModelForm):
	...
	helper.layout = Layout(
			...
			Field('image'),
			FormActions(
					Submit('submit_brewer_form', 'Save', css_class='btn-primary'),
			)
	)

class DeleteBrewerForm(forms.Form):
	helper = FormHelper()
	helper.form_class = 'form-horizontal'
	helper.layout = Layout(
		FormActions(
			Submit('submit_delete_brewer_form', 'Delete Brewer', css_class='btn-danger'),
		)
	)
...
class BeerStyleForm(forms.ModelForm):
	...
	helper.layout = Layout(
			Field('name', css_class='input-xlarge'),
			FormActions(
					Submit('submit_beer_style', 'Save', css_class='btn-primary'),
			)
	)

class DeleteBeerStyleForm(forms.Form):
	helper = FormHelper()
	helper.form_class = 'form-horizontal'
	helper.layout = Layout(
		FormActions(
			Submit('submit_delete_beer_style_form', 'Delete Beer Style', css_class='btn-danger'),
		)
	)
